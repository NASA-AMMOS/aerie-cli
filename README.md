...
4. Activate a configuration to start a session with an PlanDev host:
...
| `external_auth` | Specification for external authentication required to reach an PlanDev instance. See [Configuring for External Authentication](#configuring-for-external-authentication) for details | No       |

### Sessions and Roles

PlanDev-CLI maintains a persistent "session" with an PlanDev instance so multiple commands can run without needing to re-authenticate. To start a session, use the `activate` command:
...
- `aerie_cli.aerie_host.AerieHost`: An abstraction for an PlanDev Host, including methods for authentication and issuing requests to the PlanDev API.
...
If your application will be run by a user who may also be using the CLI, you may reduce the amount of code required to configure an PlanDev host and instead just use the active session. PlanDev-CLI provides a utility to retrieve an `AerieClient` instance from the active CLI session:
